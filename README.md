...
## 60db provider (the `sixtydb` package)

In addition to the ElevenLabs client, this repository ships a sibling package,
[`sixtydb`](sixtydb), that provides a Go client for the [60db](https://60db.ai)
text-to-speech API. ElevenLabs remains the primary provider; 60db is an
additional, opt-in provider implemented as a **standalone package** that
deliberately mirrors the elevenlabs client's shape (a `Client` value, a
configurable default client with package-level shorthand functions,
`TextToSpeech` returning audio bytes and `TextToSpeechStream` writing to an
`io.Writer`). This makes the two easy to use side by side. Like the rest of this
module, `sixtydb` has **no third-party dependencies** — including its WebSocket
client, which is implemented on the standard library.

Authentication uses a 60db API key (a bearer token for the REST endpoints, and
an `apiKey` query parameter for the WebSocket endpoint). Set it via the
`SIXTYDB_API_KEY` environment variable in your own code, or pass it directly:
...
func main() {
 client := sixtydb.NewClient(context.Background(), "your-60db-api-key", 30*time.Second)

 audio, err := client.TextToSpeech("voice-id", sixtydb.TextToSpeechRequest{
  Text: "Hello, world! This audio was generated by 60db.",
 })
 if err != nil {
  log.Fatal(err)
 }

 if err := os.WriteFile("hello.mp3", audio, 0644); err != nil {
  log.Fatal(err)
 }
 log.Println("Successfully generated audio file")
}
...
The package supports all three 60db TTS surfaces:

- **Simple HTTP** — `Synthesize` / `TextToSpeech` (`POST /tts-synthesize`). The
  endpoint returns base64 audio inside a JSON envelope; `TextToSpeech` decodes it
  and returns the raw audio bytes, while `Synthesize` also exposes the metadata
  (sample rate, duration, encoding).
- **HTTP streaming** — `TextToSpeechStream` (`POST /tts-stream`). The endpoint
  delivers newline-delimited JSON frames; the method decodes each base64 `chunk`
  and copies the raw audio to your `io.Writer`, returning on the `complete`
  frame. As with the ElevenLabs streaming method, set a large client timeout.
- **WebSocket** — `TextToSpeechWebSocket` (`wss://api.60db.ai/ws/tts`) for the
  full context protocol (`create_context` → `send_text` → `flush_context` →
  `audio_chunk`… → `flush_completed` → `close_context`). For interactive use,
  `DialWebSocket` returns a `*WSConn` you can drive frame by frame.

```go
// WebSocket synthesis, streaming raw PCM into a writer.
err := sixtydb.NewClient(ctx, "your-60db-api-key", time.Minute).
 TextToSpeechWebSocket(w, "Hello from the 60db WebSocket API.", sixtydb.WebSocketConfig{
  VoiceID:    "voice-id",
  Encoding:   sixtydb.EncodingLinear16,
  SampleRate: 24000,
 })
```

> **Note:** A few wire-level details (the exact field name carrying base64 audio
> in `/tts-stream` frames, and the WebSocket message field names) are inferred
> from the public 60db docs and decoded defensively. They may need a small
> adjustment once verified against a live API key.
